...
class S3Helper:
    ...
    def sync_streaming_data(self, project_name, last_n_days=10):
        ...
        local_files = [os.path.basename(p) for p in glob.glob(os.path.join(local_tweets_dir, '*.jsonl'))]
        if last_n_days is not None:
            self.logger.info('... of the last {} days...'.format(last_n_days))
            today = datetime.now()
            sync_prefixes = []
    ...
    def sync_annotation_data(self, project_name):
        self.logger.info('Syncing annotation data ...')
    ...
    def sync_media_data(self, project_name):
        self.logger.info('Syncing media data ...')
    ...
    def remove_old_files(self, remote_files=[], local_files=[], local_dir='.'):
        ...
        if num_files > 0:
            self.logger.info('Removing {} old files...'.format(num_files))
