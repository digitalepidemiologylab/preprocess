...
class ArgParse(object):
    ...
    def merge(self):
        parser = argparse.ArgumentParser(description='Preprocess raw data to create `data/1_merged`')
        parser.add_argument('-d', '--dtypes', choices=['original', 'anonymized', 'encrypted'], required=False, default=['original', 'anonymized'], nargs='+', help='Data source type to create (can be original, anonymized or encrypted), default: original and anonymizd')
    ...
    def clean(self):
        parser = argparse.ArgumentParser(description='Clean preprocessed data to generate `data/2_cleaned`')
        parser.add_argument('-d', '--dtypes', choices=['original', 'anonymized', 'encrypted'], required=False, default=['original', 'anonymized'], nargs='+', help='Data source type to use (can be original, anonymized or encrypted), default: original and anonymizd')
        parser.add_argument('--verbose', dest='verbose', action='store_true', help='Verbose output')
        args = parser.parse_args(sys.argv[2:])
        clean_tweets.run(dtypes=args.dtypes, verbose=args.verbose)

    def sample(self):
        parser = argparse.ArgumentParser(description='Sample cleaned data to generate `data/3_cleaned`')
        ...
        parser.add_argument('-d', '--dtype', type=str, required=False, dest='dtype', default='anonymized', nargs='?', help='Data source type to use (can be original, anonymous or encrypted), default: anonymized')
        parser.add_argument('-m', '--mode', choices=['monthly', 'random'], required=False, default='monthly', help='Sampling mode. Monthly: Try to sample evenly within months (default). Random: Sample randomly.')
    ...
    def batch(self):
        parser = argparse.ArgumentParser(description='Generate new batch for labelling. As a result a new csv will be created in `data/3_sampled/batch_{batch_id}/`')
        ...
        parser.add_argument('--ignore-previous', dest='ignore_previous', action='store_true', default=False, help='Also sample tweets from old batches which were not annotated (default: False)')
    ...
    def clean_labels(self):
        parser = argparse.ArgumentParser(description='Clean/merge labels from different batches to generate final training input')
        parser.add_argument('-s', '--selection-criterion', dest='selection_criterion', choices=['majority', 'unanimous'], required=False, default='majority', help='Can be "majority" (use majority vote, default) or "unanimous" (only select tweets with perfect agreement)')
        parser.add_argument('-l', '--min-labels-cutoff', dest='min_labels_cutoff', type=int, required=False, default=3, help='Discard all tweets having less than min_labels_cutoff annotations, default: 3')
        ...
        parser.add_argument('-m', '--mode', choices=['mturk', 'local', 'public', 'other', '*'], type=str, required=False, default='all', help='Annotation mode which was used. Can be `mturk`, `local`, `public`, `other` or `all` (default: `all`)')
        parser.add_argument('--is-relevant', dest='is_relevant', action='store_true', help='Filter tweets which have been annotated as relevant/related, default: False')
    ...
    def stats(self):
        ...
        if args.command == 'annotation':
            parser = argparse.ArgumentParser(description='Print stats about annotations')
            ...
            parser = argparse.ArgumentParser(description='Find annotators which have under-performed compared to others')
    ...
    def split(self):
        ...
        parser.add_argument('--name', type=str, required=False, default='', help='In case there are multiple cleaned labelled data output files give name of file (without csv ending), default: No name provided (works only with one file).')
    ...
    def sync(self):
        parser = argparse.ArgumentParser(description='Sync project data from S3')
